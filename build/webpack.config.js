...
alias = Object.assign(alias, pickFiles({
    id: /(functions\/[^\/]+).js/,
    pattern: SRC_PATH + '/js/functions/*'
}));

//containers
alias = Object.assign(alias, pickFiles({
    id: /(containers\/[^\/]+\/[^\/]+).js/,
    pattern: SRC_PATH + '/containers/*/*.js'
}));

//conf
alias = Object.assign(alias, pickFiles({
    id: /(conf\/[^\/]+).js/,
    pattern: SRC_PATH + '/js/conf/*.js'
}));
...
config.module.loaders = [{ test: /\.(eot|woff|woff2|svg|ttf)([\?]?.*)$/, loader: "file-loader" }]
